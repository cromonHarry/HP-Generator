...
# No.1-4, 4 user input
input1 = input("最近自分が別人と違う感じになった行動はありますか\n").strip()
input2 = input("それをじつげんするために、どんな製品やサービスを使っていますか\n").strip()
input3 = input("なぜそのような製品やサービスを使用するか\n").strip()
input4 = input("その行動を基づいて、どんな自分になりたいですか\n").strip()
...
hp_mt_2[HP_model[4]] = single_gpt(HP_model[10], hp_mt_2[HP_model[10]], HP_model[4])
hp_mt_1[HP_model[18]] = tavily_generate_answer(generate_question_for_tavily(HP_model[5], hp_mt_1[HP_model[5]], HP_model[18], 1))
# 5个先导社会问题
hp_mt_2[HP_model[1]] = user_choose_answer(list_up_gpt(HP_model[5], hp_mt_1[HP_model[5]], HP_model[1]))
hp_mt_2[HP_model[8]] = single_gpt(HP_model[1], hp_mt_2[HP_model[1]], HP_model[8])
# 5个社会问题
hp_mt_2[HP_model[3]] = user_choose_answer(list_up_gpt(HP_model[8], hp_mt_1[HP_model[8]], HP_model[3]))
# 5个人的价值观
hp_mt_2[HP_model[2]] = user_choose_answer(list_up_gpt(HP_model[3], hp_mt_1[HP_model[3]], HP_model[2]))
hp_mt_2[HP_model[9]] = single_gpt(HP_model[1], hp_mt_2[HP_model[1]], HP_model[9])
# 5个习惯化
hp_mt_2[HP_model[15]] = user_choose_answer(list_up_gpt(HP_model[2], hp_mt_1[HP_model[2]], HP_model[15]))
# 5个UX空间
hp_mt_2[HP_model[5]] = user_choose_answer(list_up_gpt(HP_model[2], hp_mt_1[HP_model[2]], HP_model[5]))
